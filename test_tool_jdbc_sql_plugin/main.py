...
def make_jdbc_sql_call(call: JdbcSqlCall, data: Dict[str, Any]) -> None:
    ...
    with connect(call['driver'], call['url'], [call['username'], call['password']], call['driver_path']) as conn:
        with conn.cursor() as cursor:
            ...
            for validate in call['validate']:
                try:
                    if not str(rows[validate['row']][validate['column']]) == str(validate['value']):
                        error(f'Expected {validate["value"]} in row {validate["row"]}, column {validate["column"]}, but was {rows[validate["row"]][validate["column"]]}')
                        assert False
                    else:
                        info(f'Validated row {validate["row"]}, column {validate["column"]} with value {rows[validate["row"]][validate["column"]]}')
                except KeyError as e:

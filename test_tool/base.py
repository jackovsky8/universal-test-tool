...
plugin_files = plugin_path.glob('*_plugin.py')
call_types = [file.name.replace('_plugin.py', '') for file in plugin_files]

CallType = Enum('CallType', [type.upper() for type in call_types])

# Import all plugins
components_from_plugin = ["default_${plugin}_call", "augment_${plugin}_call", "make_${plugin}_call"]
...
for plugin in call_types:
    components: List = [component.replace('${plugin}', plugin) for component in components_from_plugin]
    imported_components = {component.replace(f'{plugin}_', ''): getattr(plugin_module, component) for component in components}
...
def run_tests(project_path_str: str, calls_path_str: str, data_path_str:str) -> None:
    ...
    if errors == 0:
        ...
        error('There occured {} errors while testing, please check the logs'.format(errors))